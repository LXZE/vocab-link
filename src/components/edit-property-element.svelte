<script lang="ts">
  ...
  const addProperty = async () => {
    if (
      ["pos", "language"].includes(selected_property_key) &&
      newPropText != ""
    ) {
      await graphDB.createNewNode(
        selected_property_key as NodeType,
        newPropText
      );
    }
    newPropText = "";
  ...
  const confirmDeleteMultiProps = () => {
    deletePropFn = async () => {
      await Promise.all(
        selectedPropsId.map(async (prop_id) =>
  ...
  <tbody>
    ...
    <tr>
      ...
      <th>
        <input
          class="input"
          placeholder={`Add new ${selected_property_key}`}
          type="text"
          bind:value={newPropText}
        />
      </th>
      <th class="text-end">
...
<ConfirmDialog bind:this={confirmDialog} onConfirmCallback={deletePropFn} />
