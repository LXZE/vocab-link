<script lang="ts">
  ...
  const searchNodesByText = debounce(
    (queryText: string) => {
      ...
      searchResultNodes = queryNodeByText(normalizedQuery, allWordIndex, {
        limit: CANDIDATE_LIMIT,
      });
      // if no exact match then suggest to add a new word.
      if (
        normalizedQuery.length > 0 &&
        !searchResultNodes.some((res) => res.text == searchText)
      ) {
        searchResultNodes = [
          ...searchResultNodes,
          {
  ...
  const bodyKeydownHandler = (ev: KeyboardEvent) => {
    ...
    const isModifierKeyPressed =
      getAgentSystem() == "macos" ? ev.metaKey : ev.ctrlKey;
    if (
...
<form id="search-word-container" class="relative flex justify-center">
  <div class="flex relative items-center justify-center max-w-md grow">
    ...
    <input
      class="pl-10 py-2 input input-ghost w-full"
      ...
      autocomplete="off"
      spellcheck="false"
      bind:this={searchTextInputElem}
      bind:value={searchText}
      onfocus={() => (isSearchFocused = true)}
      onblur={() => (isSearchFocused = false)}
    />
    <div
      class="flex gap-1 items-center absolute right-0 pr-2 invisible sm:visible"
    >
      <kbd class="kbd kbd-sm">{getModifierKey()}</kbd>
      <kbd class="kbd kbd-sm">K</kbd>
      <button
        class={searchText.length > 0 ? "visible" : "invisible"}
        onclick={(ev) => {
          ev.preventDefault();
          searchText = "";
        }}
    ...
    >
      {#each searchResultNodes as node, idx}
        <li>
          <a
            href={null}
            class={searchCandidateIndex == idx ? "active" : ""}
            onmousedown={(ev) => {
