<script lang='ts'>
  ...
  const triggerReactivity = async (onlyForm = false) => {
    connectedForms = connectedForms;
    if (!onlyForm && $selectedNodeId)
  ...
  const formChoiceFn = async (queryText: string, excludeWords: string[]): Promise<string[]> => {
    return queryTextsByText(queryText, allForms, { excludeTexts: excludeWords });
  };

  const addRowFormHandler = () => {
    if (connectedForms.slice(-1)[0].word == null) return;
    connectedForms.push({ word: null, forms: [] });
    triggerReactivity(true);
  };
...
<div class='flex flex-col gap-2 py-2'>
      ...
      {:else}
        <div class="word-form-row">
          ...
          {#if connectedForm.word}
            <!-- if word selected then show tags input for form name  -->
            <span> = </span>
            <TagsInput class='word-form-name'
  ...
  <div class="tooltip self-start" data-tip="Add word form">
