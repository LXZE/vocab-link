<script lang="ts">
  ...
  interface Props {
    selectedForms: LinkedNode[];
  }

  let { selectedForms }: Props = $props();
  ...
  const setConnectedForms = () => {
    ...
    connectedForms = [];
    connectedForms.push({
      word: $selectedNode as LinkedNode,
      forms: $selectedNode?.forms ?? [],
    });
    selectedForms.forEach((node) =>
      connectedForms.push({ word: node, forms: node.forms ?? [] })
    );
    connectedForms = connectedForms;
  };
  run(() => {
    ($selectedNode, selectedForms), setConnectedForms();
  });
  let linkedWordNodesId = $derived(
    connectedForms
      .map((form) => form.word)
  ...
  const removeWordFormHandler =
    (wordIndex: number) => async (removedNode: number | Node) => {
  ...
  const addFormHandler =
    ...
    ({ text: form }: Pick<Node, "text">) => {
      ...
      graphDB.updateNode(
        currentWord.id,
        "forms",
        connectedForms[wordIndex].forms
      );
      triggerReactivity();
    };
  const removeFormHandler =
    (wordIndex: number) => (removedIndex: number | any) => {
      const currentWord = connectedForms[wordIndex].word;
